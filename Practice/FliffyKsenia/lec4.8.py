...
game_slovs = ['камень', 'ножницы', 'бумага','stop']
# запрашиваем у пользователя камень, ножницы, бумага
while True:

    while True:
        igrok = input("Введите камень, ножницы, бумага(если считаете, что никогда не выйграете, то ваше стоп-слово 'STOP'): ").lower()
        if igrok in game_slovs:
            break
    if igrok == game_slovs[3]:
        print("Спасибо, с Вами было приятно играть. До свидания.")
        break
